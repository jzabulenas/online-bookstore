...
test("should click like on a single generated book, and see the icon change color", async ({
  ...
  await firstBook.getByTestId(/^like-/).click(); // clicks on the like button of the first book
...
test("should click like on two generated books, and see their icons change color", async ({
  page,
}) => {
  const email = `antanas+${uuidv4()}@inbox.lt`;
  ...
  const thirdBook = books.nth(2);
  await firstBook.getByTestId(/^like-/).click(); // clicks on the like button of the first book
  await secondBook.getByTestId(/^like-/).click(); // clicks on the like button of the second book

  // Assert
  await expect(page).toHaveURL("http://localhost:5173");
  await expect(firstBook.getByTestId(/^liked-/)).toBeVisible();
  await expect(secondBook.getByTestId(/^liked-/)).toBeVisible();
  await expect(thirdBook.getByTestId(/^like-/)).toBeVisible();
  await expect(page.getByRole("main")).toMatchAriaSnapshot(`
    - main:
      - heading "Welcome, ${email}" [level=1]
...
test("should click like on three generated books, and see their icons change color", async ({
  page,
}) => {
  const email = `antanas+${uuidv4()}@inbox.lt`;
  ...
  await firstBook.getByTestId(/^like-/).click(); // clicks on the like button of the first book
  await secondBook.getByTestId(/^like-/).click(); // clicks on the like button of the second book
  await thirdBook.getByTestId(/^like-/).click(); // clicks on the like button of the third book

  // Assert
  await expect(page).toHaveURL("http://localhost:5173");
  await expect(firstBook.getByTestId(/^liked-/)).toBeVisible();
  await expect(secondBook.getByTestId(/^liked-/)).toBeVisible();
  await expect(thirdBook.getByTestId(/^liked-/)).toBeVisible();
  await expect(page.getByRole("main")).toMatchAriaSnapshot(`
    - main:
      - heading "Welcome, ${email}" [level=1]
      - heading "How to use" [level=2]
