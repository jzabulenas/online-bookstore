...
test("should reset password successfully", async ({ page }) => {
  ...
  await resetPage
    ...
    .fill(newPassword);
  await resetPage.getByRole("button", { name: "Reset password" }).tap();
  ...
  await resetPage.getByRole("button", { name: "Submit" }).tap();
...
test("should display error when token is invalid", async ({ page }) => {
  ...
  await page
    .getByRole("textbox", { name: "Confirm new password:" })
    .fill("r9$CbHEaGXLUsP");
  await page.getByRole("button", { name: "Reset password" }).tap();
