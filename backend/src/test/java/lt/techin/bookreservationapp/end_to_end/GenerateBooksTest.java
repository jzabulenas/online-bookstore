...
class GenerateBooksTest {
  ...
  void whenUserHasBooksTheyLikedBefore_thenNotSeeThemInNewlyGeneratedBooks() {
    ...
    // Generate new books
    Response newlyGeneratedBooksResponse = given()
      .cookie("JSESSIONID", logInResponse.getSessionId())
      .cookie("XSRF-TOKEN", csrfToken)
      .header("X-XSRF-TOKEN", csrfToken)
      .contentType(ContentType.JSON)
      ...
      .extract()
      .response();
    List<String> newlyGeneratedBooksList = newlyGeneratedBooksResponse
      .jsonPath()
      .getList("result");
    ...
    Response newlyGeneratedBooksResponse2 = given()
      .cookie("JSESSIONID", logInResponse.getSessionId())
      .cookie("XSRF-TOKEN", csrfToken)
      .header("X-XSRF-TOKEN", csrfToken)
      .contentType(ContentType.JSON)
      ...
      .extract()
      .response();
    List<String> newlyGeneratedBooksList2 = newlyGeneratedBooksResponse2
      .jsonPath()
      .getList("result");

    // Retrieve liked books of user
    Response existingUserBooksResponse = given()
      .cookie("JSESSIONID", logInResponse.getSessionId())
      .cookie("XSRF-TOKEN", csrfToken)
      .header("X-XSRF-TOKEN", csrfToken)
      .when()
      .get("http://localhost:8080/books");
    List<String> existingUserBooksList = existingUserBooksResponse
      .jsonPath()
      .getList("title");

    assertThat(
      Collections.disjoint(newlyGeneratedBooksList2, existingUserBooksList),
      equalTo(true)
    );
  }
