...
class UserBookService {
  ...
  private final Map<String, Boolean> bookExistsCache =
  ...
  MessageResponseDTO generateBooks(MessageRequestDTO messageRequestDTO) {
    ...
    String[] verified = Arrays.stream(
      Objects.requireNonNull(result).split("\\|")
    )
      .map(String::trim)
  ...
  private boolean lookupBook(String candidate) {
    ...
    try {
      OpenLibraryResponse response = this.restClient.get()
        .uri(
          "/search.json?title={title}&author={author}&limit=1",
          title,
          author
        )
        .retrieve()
  ...
  UserBookResponseDTO saveUserBook(
    UserBookRequestDTO userBookRequestDTO,
    Principal principal
  ) {
    ...
    if (
      this.userBookRepository.existsByBookTitleAndUser(
        userBookRequestDTO.title(),
        user
      )
    ) {
    ...
    if (this.bookRepository.existsByTitle(userBookRequestDTO.title())) {
      Book book = this.bookRepository.findByTitle(
        userBookRequestDTO.title()
      ).orElseThrow();

      UserBook savedUserBook = this.userBookRepository.save(
      ...
      Book book = this.bookRepository.save(
        new Book(userBookRequestDTO.title(), null)
      );
